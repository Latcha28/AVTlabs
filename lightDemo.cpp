...
void renderScene(void) {
	...
	for (int i = 0 ; i < myMeshes.size(); ++i) {
			...
			pushMatrix(MODEL);
			translate(MODEL, 0.0f, 0.0f, i*2.0f);
			if (i == 0) {
...
void init()
{
	...
	float water_amb[] = { 0.0f, 0.0f, 0.2f, 1.0f };
	float water_diff[] = { 0.0f, 0.0f, 0.8f, 1.0f };
	float shininess= 100.0f;
	int texcount = 0;

	// create geometry and VAO of the pawn
	amesh = createQuad(120.0f, 120.0f);
	memcpy(amesh.mat.ambient, water_amb, 4 * sizeof(float));
	memcpy(amesh.mat.diffuse, water_diff, 4 * sizeof(float));
